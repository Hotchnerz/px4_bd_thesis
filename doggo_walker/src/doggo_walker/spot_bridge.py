...
#map_tf_body = SE3Pose(0, 2.0, 0, Quat(w=1.0, x=0, y=0, z=0))
...
class SpotBodyPublisher:
    ...
    def connect(self, max_retries=5, delay=3.0):
        for attempt in range(1, max_retries + 1):
            try:
                ...
                rospy.logwarn(f"Spot unreachable at {self.hostname} (attempt {attempt}/{max_retries})")
                if attempt < max_retries:
                    rospy.sleep(delay)
        
        rospy.logerr(f"Failed to connect to Spot after {max_retries} attempts. Shutting down.")
        rospy.signal_shutdown("Spot unreachable")
        return False
